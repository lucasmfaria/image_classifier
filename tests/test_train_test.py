...
def test_train_command():
    p = subprocess.run(['python', str(Path(r'./scripts/train.py')), '--sample_dataset', 'mnist', '--base_epochs', '3',
                        '--fine_tuning_epochs', '3', '--unit_test_dataset', 'True'], shell=get_platform_shell(), check=True)
    assert p.returncode == 0
...
def test_test_command():
    ...
    assert p.returncode == 0
    p = subprocess.run(['python', str(Path(r'./scripts/delete_last_train.py'))], shell=get_platform_shell(), check=True)  # delete after model testing
